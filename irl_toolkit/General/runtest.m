...
function [w_vec,featuresMap,costMap] = runtest(algorithm,algorithm_params,...
    mdp_model,mdp,mdp_params,test_params,example_samples_Trial,features_totalTrial)
...
%% Added before to test diff lengths of trajs
%  example_samples{1,9}=[309;3];

%%

% %% ROGERIO CHANGE FEATURE DATA
feature_data.splittable=features_totalTrial;
example_samples=example_samples_Trial;
test_params.training_samples=35;
% %%
...
w_vec=irl_result.model_itr{1};
featuresMap=[features_totalTrial,ones(size(features_totalTrial(:,1)))];
costMap=w_vec'*featuresMap';
costMap=reshape(costMap,100,100);
contour(costMap);
% costMap=costMap-mean2(costMap);

%% EVALUATION OF RESULT -- REMOVED FOR THESE TESTS
% Evaluate result.
