...
%}
r_tree = struct('type',1,'test',1+step*2,'total_leaves',3,...       % Test distance to c1 1 shape
    'ltTree',struct('type',0,'index',1,'mean',zeros(1,9)),... % Neutral reward for being elsewhere
    'gtTree',struct('type',1,'test',2+step*1,'total_leaves',2,...   % Test distance to c1 2 shape
        'gtTree',struct('type',0,'index',2,'mean',1*ones(1,9)),...  % Reward for being close
        'ltTree',struct('type',0,'index',3,'mean',-2*ones(1,9))));   % Penalty otherwise
