...
sa_s = zeros(mdp_params.n^2,9,9);
sa_p = zeros(mdp_params.n^2,9,9);
for y=1:mdp_params.n,
    for x=1:mdp_params.n,
        s = (y-1)*mdp_params.n+x;
        successors = zeros(1,1,9);
        successors(1,1,1) = s; % stay
        successors(1,1,2) = (min(mdp_params.n,y+1)-1)*mdp_params.n+x; % pos y
        successors(1,1,6) = (min(mdp_params.n,y+1)-1)*mdp_params.n+max(1,x-1); % pos y neg x
        successors(1,1,3) = (y-1)*mdp_params.n+min(mdp_params.n,x+1); % pos x
        successors(1,1,7) = (min(mdp_params.n,y+1)-1)*mdp_params.n+min(mdp_params.n,x+1); % pos y pos x
        successors(1,1,4) = (max(1,y-1)-1)*mdp_params.n+x; % neg y
        successors(1,1,8) = (max(1,y-1)-1)*mdp_params.n+max(1,x-1); % neg y neg x
        successors(1,1,5) = (y-1)*mdp_params.n+max(1,x-1); % neg x
        successors(1,1,9) = (max(1,y-1)-1)*mdp_params.n+min(mdp_params.n,x+1); % neg y pos x
        sa_s(s,:,:) = repmat(successors,[1,9,1]);
        sa_p(s,:,:) = reshape(eye(9,9)*mdp_params.determinism + ...
            (ones(9,9)-eye(9,9))*((1.0-mdp_params.determinism)/4.0),...
            1,9,9);
    end;
end;
